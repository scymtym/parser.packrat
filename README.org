...
* STARTED Reference
  ...
  + =base-grammar= Grammar Class
     ...
     + <<and-expression>> ~AND-EXPRESSION ::= (LIST 'AND (* (~[[expression][~expression~]] ~)))~

     + <<anything-expression>> ~ANYTHING-EXPRESSION ::= (OR ANY 'ANY)~

     + <<compose-expression>> ~COMPOSE-EXPRESSION ::= (LIST 'COMPOSE (* (~[[expression][~expression~]] ~)))~

     + <<constant-expression>> ~CONSTANT-EXPRESSION ::= (OR (LIST 'QUOTE «anything») (GUARD «anything» (TYPEP '(NOT (OR CONS (AND SYMBOL (NOT KEYWORD)))))))~
     ...
     + <<function-name-or-partial-application>> ~FUNCTION-NAME-OR-PARTIAL-APPLICATION ::= (OR (GUARD «anything» SYMBOLP) (LIST (GUARD «anything» SYMBOLP) (REST-EXPRESSION «anything»)))~

     + <<next-rule-invocation-expression>> ~NEXT-RULE-INVOCATION-EXPRESSION ::= (LIST (OR NEXT-RULE 'NEXT-RULE) (* (~[[expression][~expression~]] ~)))~

     + <<not-expression>> ~NOT-EXPRESSION ::= (LIST 'NOT (~[[expression][~expression~]] ~))~

     + <<or-expression>> ~OR-EXPRESSION ::= (LIST 'OR (* (~[[expression][~expression~]] ~)))~

     + <<predicate-expression>> ~PREDICATE-EXPRESSION ::= (LIST GUARD (? (~[[expression][~expression~]] ~)) (REST-EXPRESSION (~[[function-name-or-partial-application][~function-name-or-partial-application~]] ~)))~

     + <<push-expression>> ~PUSH-EXPRESSION ::= (LIST (OR <<- '<<-) (~[[variable-name][~variable-name~]] ~) (? (~[[expression][~expression~]] ~)))~

     + <<rule-invocation-expression>> ~RULE-INVOCATION-EXPRESSION ::= (LIST (OR (GUARD «anything» SYMBOLP) (LIST (GUARD «anything» SYMBOLP) (GUARD «anything» SYMBOLP))) (* (~[[expression][~expression~]] ~)))~

     + <<set-expression>> ~SET-EXPRESSION ::= (OR (~[[variable-name][~variable-name~]] ~) (LIST (OR <- '<-) (~[[variable-name][~variable-name~]] ~) (? (~[[expression][~expression~]] ~))))~

     + <<transform-expression>> ~TRANSFORM-EXPRESSION ::= (LIST TRANSFORM (~[[expression][~expression~]] ~) (REST-EXPRESSION «anything»))~

     + <<variable-name>> ~VARIABLE-NAME ::= (GUARD «anything» (TYPEP '(AND SYMBOL (NOT (OR KEYWORD NULL)))))~

  + =sequence-grammar= Grammar Class

     NIL

     + <<+-expression>> ~+-EXPRESSION ::= (LIST '+ «anything»)~

     + <<?-expression>> ~?-EXPRESSION ::= (LIST '? «anything»)~

     + <<bounds-expression>> ~BOUNDS-EXPRESSION ::= (LIST 'BOUNDS (LIST (GUARD «anything» SYMBOLP) (GUARD «anything» SYMBOLP)) (REST-EXPRESSION «anything»))~
     ...
     + <<repetition-expression>> ~REPETITION-EXPRESSION ::= (LIST '* (~[[expression][~expression~]] ~) (? (~[[expression][~expression~]] ~)) (? (~[[expression][~expression~]] ~)))~

     + <<sequence-expression>> ~SEQUENCE-EXPRESSION ::= (LIST (OR SEQ 'SEQ) (* (~[[expression][~expression~]] ~)))~
  ...
  + =sexp-grammar= Grammar Class
     ...
     + <<cons-expression>> ~CONS-EXPRESSION ::= (LIST 'CONS «anything» «anything»)~
     ...
     + <<list*-expression>> ~LIST*-EXPRESSION ::= (LIST 'LIST* (* (AND (SEQUENCE-EXPRESSION («anything» «anything»)) «anything»)) «anything»)~

     + <<list-elements-expression>> ~LIST-ELEMENTS-EXPRESSION ::= (LIST 'LIST-ELEMENTS (~[[expression][~expression~]] ~))~

     + <<list-expression>> ~LIST-EXPRESSION ::= (LIST 'LIST (REST-EXPRESSION «anything»))~

     + <<rest-expression>> ~REST-EXPRESSION ::= (LIST 'REST (~[[expression][~expression~]] ~))~

     + <<structure-expression>> ~STRUCTURE-EXPRESSION ::= (LIST 'STRUCTURE (~[[expression][~expression~]] ~) (* (LIST «anything» (~[[expression][~expression~]] ~))))~

     + <<value-expression>> ~VALUE-EXPRESSION ::= (LIST 'VALUE (LIST (GUARD «anything» SYMBOLP) (~[[expression][~expression~]] ~)))~

     + <<vector*-expression>> ~VECTOR*-EXPRESSION ::= (LIST 'VECTOR* (REST-EXPRESSION «anything»))~
